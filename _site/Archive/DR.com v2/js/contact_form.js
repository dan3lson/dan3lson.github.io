$(document).ready(function() {
	...
	validate_dropdown_field_live("#cf-product");
	validate_submission_by_human("#prove-ur-human-field", $random_string);

	$("#website-cta-btn, #web-app-cta-btn").click(function() {
		scroll_to("#form-container");
		change_dropdown_value(this, "#cf-product");
	});
	...
	function validate_dropdown_field_live(dropdown_selector) {
		$(dropdown_selector).change(function() {
			var $selection = $(dropdown_selector).val();

			if ($selection != "") {
				display_passing_border_color(dropdown_selector);
				validate_form_fields("#cf-submit-btn");
			} else {
				display_failing_border_color(dropdown_selector);
				validate_form_fields("#cf-submit-btn");
			}
		});
	}
